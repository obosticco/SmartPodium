...
class Cubby {
private:
    byte identifier[16];  // Fixed-size array
    int lockPin;
    bool vacant;
    int CubbyNum;
...
public:
    ...
    Cubby(byte id[16], int pin, int num) {
        memcpy(identifier, id, sizeof(identifier));  // Initialize identifier
        lockPin = pin;  // Initialize lockPin
        vacant = true;  // Initialize vacant (assumed vacant initially)
...
void setup() {
    ...
    Cubby cubbies[2] = {
        Cubby(CUBBY1DATA, 3, 1),
        Cubby(CUBBY2DATA, 4, 2)
    };
}
